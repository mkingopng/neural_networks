...
def main():
    ...
    for epoch in range(student.epochs):
        ...
        for i, batch in enumerate(trainLoader):
            ...
            if i % 32 == 31:
                # wandb.log("metric"=runningLoss/32)
    ...
    if student.trainValSplit != 1:
        ...
        with torch.no_grad():
            for batch in valLoader:
                ...
                correctRatingOnlySum += torch.sum(
                    correctRating & ~correctCategory).item()
                correctCategoryOnlySum += torch.sum(
                    correctCategory & ~correctRating).item()
                bothCorrectSum += torch.sum(
                    correctRating & correctCategory).item()
